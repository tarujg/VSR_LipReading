...
class MyDataset(Dataset):
    ...
    @staticmethod
    def sentences(predict, truth):
        correct_sentences = len(list(set(predict) & set(truth)))
        total_sentences = len(truth)
        return correct_sentences, total_sentences

    @staticmethod
    def wla(predict, truth):
        word_pairs = [(p[0].split(' '), p[1].split(' ')) for p in zip(predict, truth)]
        wla = [(1.0 - 1.0 * editdistance.eval(p[0], p[1]) / len(p[1])) for p in word_pairs]
        return wla
